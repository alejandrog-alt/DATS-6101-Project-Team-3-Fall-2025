...
*Outlier Check for the variables: weekly study hours, attendance, class participation and total score. We use the ezids package*
...
```{r}
studperf$attendance_group <- ifelse(studperf$attendance_percentage >= 85, 
                                   "High (≥85%)", "Low (<85%)")
```
```{r}
# Create a new categorical variable for attendance
studperf$attendance_group <- ifelse(studperf$attendance_percentage >= 85, "High (≥85%)", "Low (<85%)")

<<<<<<< HEAD
table(studperf$attendance_group)

```

Calculate averages for attendance percentage >= 85 and attendance percentage < 85
```{r}
...
Calculating average total scores for high attendace group 
>>>>>>> 2351712 (Updated Q1 analysis based on t-test for attendance_group as categorical value)
```{r}
high_mean <- mean(subset(studperf, attendance_group == "High (≥85%)")$total_score)
high_mean
```

<<<<<<< HEAD
**Answer:** The students with attendance ≥ 85% have an average total score of `r mean(high_attend$total_score)` compared to `r mean(low_attend$total_score)` for students with attendance < 85%.Therefore, it is not really true that students with attendance greater or equal to 85% perform better than those with attendance less than 85%.
=======
Calculating average total scores for low attendace group 
```{r}
low_mean  <- mean(subset(studperf, attendance_group == "Low (<85%)")$total_score)
low_mean
```
>>>>>>> 2351712 (Updated Q1 analysis based on t-test for attendance_group as categorical value)
