...
>>>>>>> e6c4a18dc03de122a33cf79d12406b4271c6874e
...
Simple linear regression 
```{r}
model <- lm(total_score ~ weekly_self_study_hours, data = studperfclean4)
slope <- coef(model)[2]
p_value <- summary(model)$coefficients[2, 4]
```
...
Plot
```{r}
ggplot(studperfclean4, aes(x = weekly_self_study_hours, y = total_score)) +
  geom_point(alpha = 0.6) +
  geom_smooth(method = "lm", color = "blue") +
  labs(
    title = "Relationship between Study Hours and Total Score",
